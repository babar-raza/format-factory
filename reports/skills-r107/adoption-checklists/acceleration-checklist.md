# Acceleration Adoption Checklist (Skills R107) -- Enforcement Gates

This checklist supersedes the R106 version. All R106 gates are retained.
R107 additions are marked with **(R107 NEW)** and close the gap between
documented rules and machine-enforced validation.
...
### R107 Registry Stability Enforcement (R107 NEW)

- Lane C registry stability tests guarantee that the routing table above
  remains accurate. If a skill_id referenced in a handoff is not `status: active`
  in the registry, the handoff is invalid.
- **Testable assertion:** For every `skill_id` in the routing table, the
  registry MUST contain an entry with `status: active`. Registry stability
...
## GATE A-07: POC Items Must Reference Registered Skills (REQUIRED) (R107 NEW)

- **What:** Every POC work item (gap promotion, capability expansion, dogfood
  proof) that will eventually require product source changes MUST identify the
  registered skill_id that the downstream Mainstream worker will use.
- **Check:** POC work item in the evidence declaration contains a
  `target_skill_id` field. The value MUST match an `status: active` entry in
  `.supervisor/skill-registry.yaml`.
- **Validator:** `inspect_declared_evidence.py` checks POC-tagged items for
  `target_skill_id`. If present, cross-references against the registry.
- **Failure:**
  - `target_skill_id` absent on POC item => warning (non-blocking but logged
    as quality risk; capped at ACCEPTED_WITH_LIMITATIONS).
  - `target_skill_id` present but not in registry => OVERCLAIMED --
    "POC item references unregistered skill."

| Condition | Pass/Fail |
|-----------|-----------|
| POC item with `target_skill_id` matching active registry entry | PASS |
| POC item with `target_skill_id` not in registry | FAIL (OVERCLAIMED) |
| POC item without `target_skill_id` | WARN (ACCEPTED_WITH_LIMITATIONS) |
| Non-POC item | N/A (gate does not apply) |

### Why This Gate Exists

R106 enforced that acceleration items cannot edit source (A-04) and must
route through skills (A-02), but did not require POC planning items to
pre-identify which skill would be used downstream. This created a planning
gap: POC items were accepted without confirming that a suitable skill exists
in the registry, leading to handoffs that referenced nonexistent skills.
A-07 closes this by requiring forward-reference to a registered skill at
POC time.

### Integration with Lane C Registry Stability

Lane C registry stability tests guarantee that `status: active` skills have
valid command files and required handoff fields. A-07 leverages this by
ensuring POC items only reference skills that pass stability checks. If a
skill fails stability tests, any POC item referencing it will also be
flagged during the next grading cycle.
...
# Full grading (supervisor runs this -- enforces A-04 and A-07)
...
|------|--------------------|----------------|
| A-02 | Handoff `skill_id` not in registry => OVERCLAIMED | `validate_claude_commands.py` + handoff check |
| A-04 | Acceleration item with `src/` path => REJECTED | `inspect_declared_evidence.py` |
| A-07 | POC item with unknown `target_skill_id` => OVERCLAIMED | `inspect_declared_evidence.py` |
