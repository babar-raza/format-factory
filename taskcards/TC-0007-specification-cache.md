...
release_blockers: []
notes: Phase 1 infrastructure taskcard — implements generic specification cache tooling defined in docs/specification-cache.md. No format-specific spec download is required or performed by default. Real spec acquisition for any format requires a separate explicit execution prompt authorization.
---
...
| Legal policy | `docs/legal-and-licensing.md` | Required (legal category check before any real acquisition) |

---

## Steps

1. Read `docs/specification-cache.md` in full. Understand the spec-index.yaml schema and all acquisition rules, including the authorization-required rules.
2. Implement `tools/spec-cache/spec_index.py`: read/write/validate spec-index.yaml; include the full schema from `docs/specification-cache.md`; raise on schema violations.
