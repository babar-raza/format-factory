...
FODP uses the same governing specification as FODS and FODT:
...
No new spec download is required. The FODP flat-XML format is defined within the
same ODF 1.3 schema; FODP files are presentation documents conforming to the
`application/vnd.oasis.opendocument.presentation-flat-xml` MIME type.
...
## FODP-Specific Relevance

- FODP is a flat-XML encoding of ODP (OpenDocument Presentation)
- File structure: single XML file, no ZIP container
- Root element: `<office:document>` with `office:mimetype="application/vnd.oasis.opendocument.presentation-flat-xml"`
- Governed by ODF 1.3 schema — same schema governs FODS (spreadsheet) and FODT (text)
...
4. FODS Gate 2 cache is authoritative for FODP Gate 2
