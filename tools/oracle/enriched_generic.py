...
#
# TC-ORACLE-002 (libxliff Mission 2): xliff now points at the installed,
# independent `libxliff` package instead of the superseded
# `format_factory.xliff` donor. libxliff is the maintained product going
# forward (see format-factory's libxliff extraction plan); the donor under
# src/python/xliff/ is frozen legacy history, not touched by this change.
# libxliff must be `pip install`-ed into this venv for the oracle to run —
# it is test-only tooling here, never a runtime dependency of anything else
# in this repository.
ENRICHED_LOADERS = {
...
def _enrich_model(result_val, format_id: str) -> dict:
    ...
    if format_id == "safetensors":
        ...
        # libxliff has no spec_qname concept (deliberately removed as an
        # FF-monorepo-only convention during extraction — see libxliff's
        # ADR/TC-API-002-03); leave it None rather than fabricate one.
        del enriched["spec_qname"]
        enriched["unit_count"] = result_val.unit_count
        enriched["version"] = result_val.version
    elif format_id == "ora":
...
def execute_enriched_generic(core, case, pkg, format_id, module, callable_name):
    ...
    try:
        ...
        module_path = core.resolve_executor_module_path(module)
