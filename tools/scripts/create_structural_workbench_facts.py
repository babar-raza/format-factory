...
def make_workbench_yaml(fmt: str, cfg: dict) -> str:
    ...
    lines = [
        ...
        "# These are verified_with_note: structurally obvious from spec, not PDF-text-extracted.",
        "authority_note: >-",
        f"  Structural facts for {fmt.upper()} ({cfg['spec_body']}).",
        "  Verified_with_note: structurally derived from observed format elements.",
        f"  Generated: {NOW}",
    ...
    for fact in facts:
        lines.extend([
            ...
            "    chunk_id: null",
            "    confidence: medium",
            "    created_by: create_structural_workbench_facts.py",
            ...
            "    spec_page_confirmed: false",
            f"    updated_at: '{NOW}'",
            f"    validated_at: '{NOW}'",
            "    validated_by: structural_derivation",
