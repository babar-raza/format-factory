...
def benchmark_ubl():
    """Benchmark ubl parse (load) and write (dumps)."""
    from format_factory.ubl import load, dumps

    sample = REPO_ROOT / "samples" / "by-format" / "ubl" / "valid" / "invoice-full-depth.xml"
    if not sample.exists():
        candidates = list((REPO_ROOT / "samples" / "by-format" / "ubl" / "valid").rglob("*.xml"))
        if candidates:
            sample = candidates[0]
        else:
            return {"error": "No ubl sample found"}
