...
public sealed class FodtDocument
{
    ...
    // -------------------------------------------------------------------------
    // R115 — Outline JSON export + paragraph-style filter
    // -------------------------------------------------------------------------
    ...
    public string ExportToOutlineJson()
    {
        ...
        for (int i = 0; i < paragraphs.Count; i++)
        {
            ...
            // Heading level from text:outline-level attribute (text:h elements only)
            var levelAttr = p.Element.Attribute(NsText + "outline-level")?.Value;
            int level = 0;
            if (levelAttr != null && int.TryParse(levelAttr, out int parsed))
                level = parsed;
            // Fallback: detect heading from style name if outline-level not set
    ...
    public IReadOnlyList<int> FindParagraphsByStyle(string stylePattern)
    {
        ...
        for (int i = 0; i < paragraphs.Count; i++)
        {
            ...
            // Heading elements (text:h) that lack a style-name get synthetic "Heading" style
