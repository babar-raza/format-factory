"""
tabular_document.py — TSV tabular-document domain module.

Whole-document metric and summary operations over the parsed TSV tabular document
(row/column counts, densities, field-count summaries, value-type checks). Each
function accepts a file path or parsed TSV data and describes a property of the
document as a whole — the sanctioned domain-module role for a ``*_document.py``
file (see tests/test_source_structure.py). Parser source: tsv_parser.py
(parse_tsv_strict); spec concept: IANA text/tab-separated-values row/cell.

Split for size under TC-PA-017: lower-level row metrics live in the companion
tsv_row_metrics.py, re-exported below so every public name stays importable from
this module. (Originally seeded by tsv_parser.py extraction, TC-HEAL-FORMATS-BATCH1.)
"""
...
# ---------------------------------------------------------------------------
from .tsv_row_metrics import *  # noqa: F401,F403,E402
