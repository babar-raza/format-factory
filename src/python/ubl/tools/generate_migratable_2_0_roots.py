"""Generate the pinned UBL-2.0-to-2.3 migration-eligible root-type table.

Dev-time-only generator. Diffs the acquired OASIS UBL 2.0 release package
(SRC-UBL-007) against the already-vendored UBL 2.3 maindoc schemas
(SRC-UBL-002) to determine, once, which of the 91 UBL 2.3 root document
types have a UBL 2.0 maindoc schema whose root-level element sequence
...
The UBL 2.0 package uses a different internal layout than the 2.1/2.2/2.3
packages: its maindoc schemas live under ``os-UBL-2.0/xsd/maindoc/`` (a
top-level ``os-UBL-2.0/`` directory prefix the later packages do not have),
and it defines maindoc schemas for only 62 of the 91 UBL 2.3 root types --
UBL 2.0 is the oldest release and predates the introduction of many later
document types entirely.

Requires the acquired UBL 2.0 package to be materialized locally
(``python -m tools.format_contract.authority_materializer materialize
--format ubl``) since 2.0 is not bundled with the shipped wheel -- only
...
_ACQUIRED_2_0 = _REPO_ROOT / ".local" / "format-contracts" / "acquired" / "ubl" / "src-ubl-007.bin"
...
def _is_migration_eligible(seq20: list[tuple[str, str, str]], seq23: list[tuple[str, str, str]]) -> bool:
    names20 = [x[0] for x in seq20]
    names23 = [x[0] for x in seq23]
    if [n for n in names20 if n not in names23]:
        return False  # an element was removed between 2.0 and 2.3
    if [n for n in names20 if n in names23] != [n for n in names23 if n in names20]:
        return False  # shared elements were reordered
    d20 = {x[0]: x for x in seq20}
    d23 = {x[0]: x for x in seq23}
    for name in names20:
        before, after = d20[name][1:], d23[name][1:]
        if before == after:
            continue
    ...
    for name in names23:
        if name not in names20 and d23[name][1] != "0":
...
def _compute_manifest() -> dict[str, object]:
    ...
    with (
        zipfile.ZipFile(_ACQUIRED_2_0) as zf20,
        zipfile.ZipFile(_ACQUIRED_2_3) as zf23,
    ):
        authority_sha256_2_0 = hashlib.sha256(_ACQUIRED_2_0.read_bytes()).hexdigest()
        authority_sha256_2_3 = hashlib.sha256(_ACQUIRED_2_3.read_bytes()).hexdigest()
        eligible: list[str] = []
        for name in ROOT_NAMES:
            seq20 = _root_sequence(zf20, f"os-UBL-2.0/xsd/maindoc/UBL-{name}-2.0.xsd")
            seq23 = _root_sequence(zf23, f"xsd/maindoc/UBL-{name}-2.3.xsd")
            if seq20 is None or seq23 is None:
                continue  # no UBL 2.0 schema for this root type at all
            if _is_migration_eligible(seq20, seq23):
    ...
    return {
        "authority_sha256_2_0": authority_sha256_2_0,
...
def _render(manifest: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        f'AUTHORITY_SHA256_2_0: Final = "{manifest["authority_sha256_2_0"]}"',
...
def main() -> int:
    ...
    manifest_path = package / "_generated" / "migratable_2_0_roots_manifest.json"
    output_path = package / "_generated" / "migratable_2_0_roots.py"
