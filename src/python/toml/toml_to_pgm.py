"""
toml_to_pgm.py — Dogfood export: TOML → PGM using Format Factory libraries.

Sprint: TOML-TO-PGM-DOGFOOD-001
Authority: TASK-007 -- Advance one dogfood export path
Ledger entry: R90-TOML-TO-PGM-DOGFOOD-001
License: Apache-2.0
"""
...
def toml_to_pgm(
    ...
    """Convert a TOML file to a PGM bitmap, one grayscale-row per data row."""
    src_path = Path(src_path)
    dest_path = Path(dest_path)
    dest_path.parent.mkdir(parents=True, exist_ok=True)

    data = load_toml(src_path)  # Format Factory toml reader
    grid = []
    if isinstance(data, dict):
        for key, val in data.items():
            grid.append([str(key), str(val)])
