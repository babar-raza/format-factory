"""
fodp_to_csv.py — Dogfood export: FODP → CSV using Format Factory libraries.

Reads a Flat OpenDocument Presentation (.fodp) file using Format Factory's
FODP codec and writes each presentation slide as a CSV row using Format
Factory's CSV writer.

Each slide becomes one row with columns: slide_name, title, text_content.

Sprint: FODP-TO-CSV-DOGFOOD-001
Authority: TASK-007 -- Advance one dogfood export path
Ledger entry: R90-FODP-TO-CSV-DOGFOOD-001
...
def fodp_to_csv(
    ...
    *,
    ...
    """Convert a FODP presentation to CSV, one row per slide.

    Each slide becomes one CSV row with columns slide_name, title,
    ...
    if include_slide_index:
        headers.append("slide_index")
    headers.extend(["slide_name", "title", "text_content"])
