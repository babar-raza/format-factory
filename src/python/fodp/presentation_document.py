"""
presentation_document.py — spec-owned FODP presentation domain module.

Skill invocation: decompose-monolithic-codec
Format: FODP
Sprint: vivid-napping-kurzweil (TC-VNK-008)
Ledger entry: VNK-008-FODP-ANALYTICS-EXTRACT

This module contains analytics functions that operate on FODP presentations
using only the public API of fodp_codec (load, get_page_count, extract_text).
No direct ElementTree or XML parsing is performed here.

Usage:
    from presentation_document import fodp_slide_titles, fodp_text_per_slide, ...

These functions are also re-exported via fodp_codec (from .presentation_document import *)
...
def fodp_slide_titles(source: "str | bytes | Any") -> "list[str | None]":
...
def fodp_master_page_count(source: "str | bytes | Any") -> int:
...
def fodp_text_per_slide(source: "str | bytes | Any") -> list[str]:
...
def fodp_average_shapes_per_slide(source: "str | bytes | Any") -> float:
