...
Parses P3 (ASCII) PPM files: magic, width, height, maxval, RGB pixels.
...
def parse_ppm_strict(file_path: str | Path) -> PpmImage:
    """Parse a PPM file, raising PpmError on any problem."""
    path = Path(file_path)
    if not path.exists():
    ...
    if magic == "P6":
        raise PpmDecodeError("P6 (binary) format not yet supported — P3 ASCII only")
