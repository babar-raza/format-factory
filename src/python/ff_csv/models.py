...
class CsvDocument:
    ...
    # Tabular dimension properties (SAL-CSV-00001)
    ...
    # Additional tabular analysis properties (SAL-CSV-00001)
    ...
    # Scale classification properties (SAL-CSV-00001 R1233)
    ...
    # Density and shape properties (SAL-CSV-00001 R1253)
    ...
    # Extended grid analysis properties (SAL-CSV-00001 R1273)
    ...
    # Grid shape and fill completeness properties (SAL-CSV-00001 R1289)
    ...
    def add_row(self, row: list[str]) -> None:
        ...
        Raises:
            CsvError: If row is None.
        """
        from .exceptions import CsvError
        if row is None:
            raise CsvError("row must not be None")
    ...
    def set_cell(self, row_index: int, col_index: int, value: str) -> None:
        ...
        Raises:
            CsvError: If row_index or col_index is out of range.
        """
        from .exceptions import CsvError
        rows = self._data.get("rows", [])
        if row_index < 0 or row_index >= len(rows):
            raise CsvError(f"row_index {row_index} out of range (0..{len(rows) - 1})")
        row = rows[row_index]
        if col_index < 0 or col_index >= len(row):
            raise CsvError(f"col_index {col_index} out of range (0..{len(row) - 1})")
        rows[row_index][col_index] = str(value)

    def save_to_file(self, path: "str | Path") -> None:
        """Save this document to a .csv file.

        Raises:
            CsvError: If path is empty or write fails.
        """
        from .exceptions import CsvError
        from .csv_writer import write_csv_to_file
        if not path:
            raise CsvError("path must not be empty")
        dest = Path(path)
        headers = self._data.get("headers") or None
        write_csv_to_file(self._data.get("rows", []), dest, headers=headers)
