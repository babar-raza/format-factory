"""
csv_to_pgm.py — Dogfood export: CSV → PGM using Format Factory libraries.

Sprint: CSV-TO-PGM-DOGFOOD-001
Authority: TASK-007 -- Advance one dogfood export path
Ledger entry: R90-CSV-TO-PGM-DOGFOOD-001
License: Apache-2.0
"""
...
sys.path.insert(0, str(_REPO / "src" / "python" / "src/python/csv"))
...
def csv_to_pgm(
    ...
    *,
    ...
    """Convert a CSV file to a PGM bitmap, one grayscale-row per data row."""
    src_path = Path(src_path)
    dest_path = Path(dest_path)
    dest_path.parent.mkdir(parents=True, exist_ok=True)

    result = parse_csv_strict(src_path)  # Format Factory csv reader
