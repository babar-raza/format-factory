...
**Reconciliation note (2026-07-28):** This section was imported from the
preserved `codex/ff-plan-control` branch. Its original verification claims and
commit references are retained as historical evidence only. It does not assert
that the current checkout, six-format production program, or any release/gate
state is presently approved; those states remain computed from current
contracts, proof, and applicable gate records.
