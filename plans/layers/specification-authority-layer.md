...
layer_metadata:
  ...
  maturity_current: 3
  maturity_target: 5
  current_stage: PIPELINE_HARDENING
  current_owner: null
  agent_type: null
  ...
  next_action: Activate 17 dormant SAL tools; run spec extraction for all 20 Python
    FOSS formats
  handoff_id: null
```
...
This plan is the canonical working plan for **Specification Authority Layer** (`L01`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns legally acquired format specifications, deterministic extraction of spec facts, provenance, and the authority boundary for every downstream feature. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- spec acquisition records and legal source metadata
- deterministic spec-fact extraction into SAL fact registries
- spec clause to QName/obligation/capability traceability
- negative control handling for unverifiable or manually seeded facts

## 4. Explicit Non-Scope

- product source mutations
- consumer API naming choices not derived from spec facts

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for SAL/specification truth ingestion and fact registry.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `PIPELINE_HARDENING`.
- Maturity: `3/5`.
- Existing evidence paths: `['plans/strategic/snoopy-juggling-seal.md', 'reports/layer-audit-2026-06-26/forensic-layer-discovery-report.md']`.

## 10. Current Execution Stage

`PIPELINE_HARDENING`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- Earlier SAL pilots proved the concept, but memory notes a critical finding: spec facts were manually seeded in places and must be generated deterministically.
- Fact existence registry and provenance-chain enforcement exist, but all dormant tools need activation and backfill across formats.
- SAL must feed RCAL and the feature compiler directly; no human-authored shortcut facts are acceptable.
...
Current skill IDs: `['ingest-spec-sal', 'sal-pipeline-heal']`.

Current command IDs: `['ingest-spec-sal', 'sal-pipeline-heal']`.
...
Ready taskcards from metadata: `['TC-SAL-001']`.

Primary next task: `TC-SAL-001`.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-SAL-001"
  next_action: "Activate dormant SAL tools, generate fact registries from source specs, and backfill provenance for FODS, FODT, ZST, QOI, Netpbm, ABW, Gnumeric, NDJSON, TSV, SYLK and DIF."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-SAL-001`.
...
Layer-specific next action: **Activate dormant SAL tools, generate fact registries from source specs, and backfill provenance for FODS, FODT, ZST, QOI, Netpbm, ABW, Gnumeric, NDJSON, TSV, SYLK and DIF.**
