...
layer_metadata:
  ...
  next_action: "Define rollback protocol per CLAUDE.md \xA7GOV_BLOCK; close SKILL-GAP-011"
```

## 1. Layer Metadata

This plan is the canonical working plan for **Recovery and Rollback Layer** (`L25`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns safe rollback from failed sprints, bad generated code, broken releases, and live content incidents. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- rollback plans
- branch and patch recovery
- release rollback evidence
- incident containment states

## 4. Explicit Non-Scope

- blind deletion or migration without verification

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for recovery, rollback, and incident containment.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `RECOVERY_DESIGN`.
- Maturity: `2/4`.
- Existing evidence paths: `[]`.

## 10. Current Execution Stage

`RECOVERY_DESIGN`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- User treats live site/content incidents as production problems requiring containment and verification.
- Autonomous trains need rollback points before broad refactors.
- Release and package tasks need rollback/disable plans.
...
Current skill IDs: `['rollback-and-recovery']`.

Current command IDs: `['rollback-and-recovery']`.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-REC-001"
  next_action: "Define rollback checkpoints for source refactor, package release, and content/healing sprints."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-REC-001`.
...
Layer-specific next action: **Define rollback checkpoints for source refactor, package release, and content/healing sprints.**
