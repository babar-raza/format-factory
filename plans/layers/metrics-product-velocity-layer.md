...
layer_metadata:
  ...
  next_action: Define velocity metrics from grading-history.jsonl; build maturity-trend
    automation
```

## 1. Layer Metadata

This plan is the canonical working plan for **Metrics and Product Velocity Layer** (`L24`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns honest progress metrics across formats, stages, tests, maturity, accepted/rework outcomes, and product velocity. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.
...
- accepted/rework/blocker tracking

## 4. Explicit Non-Scope

- inflated completed-product claims
- marketing-only metrics

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for velocity, maturity, and product progress metrics.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `METRICS_DESIGN`.
- Maturity: `1/4`.
- Existing evidence paths: `[]`.

## 10. Current Execution Stage

`METRICS_DESIGN`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.

## 11. Current Maturity Assessment

Maturity is currently **1**. This means the layer has enough structure to guide work, but it still needs stronger proof, backfill, automation, or registry enforcement before it can be treated as fully production-grade.
...
- User rejected inaccurate claim that 20 products were successfully processed; products are at different stages.
...
Effort depends on upstream availability: `['L08', 'L11']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-MET-001"
  next_action: "Build a stage-aware product matrix showing each format, platform, maturity, gaps, tests, and next taskcards."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-MET-001`.
...
Layer-specific next action: **Build a stage-aware product matrix showing each format, platform, maturity, gaps, tests, and next taskcards.**
