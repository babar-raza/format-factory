...
layer_metadata:
  ...
  next_action: Audit __init__.py exports for all formats; define public API surface
    contract
```

## 1. Layer Metadata

This plan is the canonical working plan for **Consumer API Layer** (`L19`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns the developer-facing API: imports, public classes, method naming, stability, examples, and backwards compatibility. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.
...
- unreviewed experimental APIs

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for public API surface.
...
`API_CONTRACT`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- Public API surface contract is not formal.
...
Effort depends on upstream availability: `['L06', 'L18']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-CAPI-001"
  next_action: "Audit exports for all formats and define stable public API contract tied to capability levels."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-CAPI-001`.
...
Layer-specific next action: **Audit exports for all formats and define stable public API contract tied to capability levels.**
