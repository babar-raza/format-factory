...
This plan is the canonical working plan for **Skills Layer** (`L13`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns the rule that all governed work must run through registered skills or stop and create a skill gap report. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- skill registry and command sync
- coverage checks
- skill execution receipts
- skill idempotency and duplicate detection

## 4. Explicit Non-Scope

- ad hoc implementation when a required skill is missing

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for skill-first execution registry.
...
- Maturity: `4/5`.
- Existing evidence paths: `['.supervisor/skill-registry.yaml', '.supervisor/skill-inventory.yaml', '.supervisor/skill-quality-matrix.yaml']`.
...
- Production incident/localization work exposed missing skill coverage; gap reports must become new registered skills before execution.
- Skills must cover SAL, RCAL, feature compilation, source mutation, validation, and release boundaries.
- Skill receipts must be collected as first-class evidence.
...
Current skill IDs: `['check-skill-coverage', 'sync-skill-command-registry', 'normalize-skill-registry', 'inventory-skills', 'enforce-skill-first-execution', 'validate-skill-transcript', 'collect-skill-execution-receipts', 'preflight-skill-entry', 'detect-duplicate-skills', 'run-skill-idempotency']`.

Current command IDs: `['check-skill-coverage', 'sync-skill-command-registry', 'normalize-skill-registry', 'inventory-skills', 'enforce-skill-first-execution', 'validate-skill-transcript']`.
...
Ready taskcards from metadata: `['TC-SKILL-001']`.

Primary next task: `TC-SKILL-001`.
...
layer_session_handoff:
  ...
  next_action: "Backfill missing skills for feature compilation, source architecture refactor, and live incident/content repair patterns."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-SKILL-001`.
...
Layer-specific next action: **Backfill missing skills for feature compilation, source architecture refactor, and live incident/content repair patterns.**
