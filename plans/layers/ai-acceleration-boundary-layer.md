...
layer_metadata:
  ...
  maturity_current: 2
  maturity_target: 4
  current_stage: BOUNDARY_HARDENING
  current_owner: null
  ...
  next_action: 'BLOCKED_EXTERNAL: Gate 11 G11-G execution requires Babar Raza commercial
    sign-off'
```

## 1. Layer Metadata

This plan is the canonical working plan for **AI Acceleration Boundary Layer** (`L21`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns what agents may do autonomously, what must stop for human/external authority, and how AI-only claims are rejected. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- external gate classification
- human task conversion
- AI-only authority rejection
- post-plan terminal handling

## 4. Explicit Non-Scope

- using human gates as excuse for unverified agent work

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for AI vs human authority boundary.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `BOUNDARY_HARDENING`.
- Maturity: `2/4`.
- Existing evidence paths: `[]`.

## 10. Current Execution Stage

`BOUNDARY_HARDENING`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- Gate 11 commercial sign-off remains a true external gate.
- Agents must continue all verifiable work before asking for human review.
- AI-only proof is not acceptable for spec facts, legal claims, or release certification.
...
Current skill IDs: `['check-gate', 'check-release-boundary']`.

Current command IDs: `['check-gate', 'check-release-boundary']`.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-AIB-001"
  next_action: "Clarify blocked-external vs continue-autonomous work and encode it in taskcard gates."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-AIB-001`.
...
Layer-specific next action: **Clarify blocked-external vs continue-autonomous work and encode it in taskcard gates.**
