...
layer_metadata:
  ...
  maturity_current: 2
  maturity_target: 4
  current_stage: REGRESSION_DESIGN
  current_owner: null
  ...
  next_action: Define compatibility matrix per format; audit existing regression test
    coverage
```

## 1. Layer Metadata

This plan is the canonical working plan for **Regression Compatibility Layer** (`L17`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns regression safety: preventing existing parse/save/API behavior from breaking while architecture deepens. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.
...
- freezing bad architecture forever
- accepting silent behavior changes

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for backwards compatibility and regression control.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `REGRESSION_DESIGN`.
- Maturity: `2/4`.
- Existing evidence paths: `[]`.

## 10. Current Execution Stage

`REGRESSION_DESIGN`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- Refactors from monolithic code to object models need strong regression baselines.
...
Effort depends on upstream availability: `['L06', 'L07']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-REG-001"
  next_action: "Create regression baselines before refactoring major pilot formats."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-REG-001`.
...
Layer-specific next action: **Create regression baselines before refactoring major pilot formats.**
