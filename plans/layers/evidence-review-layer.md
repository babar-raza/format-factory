...
This plan is the canonical working plan for **Evidence and Review Layer** (`L08`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns declared evidence, materialized evidence bundles, reviewer verdict packets, and proof traceability. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- .local/evidences run folders
- evidence-declaration.yaml schema
- review packets and verdicts
- gap reconciliation maps

## 4. Explicit Non-Scope

- accepting unverified summaries
- product mutation authority

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for evidence bundles and reviewer verdicts.
...
- Maturity: `4/5`.
- Existing evidence paths: `['.supervisor/schemas/evidence-declaration.schema.json']`.
...
- Evidence packaging should not block execution unless it affects proof, but missing provenance-chain fields must be repaired.
- Reviewer must verify artifacts and tests, not trust agent prose.
- Evidence must distinguish accepted, accepted-with-rework, blocked-external, and failed states.
...
Current skill IDs: `['build-evidence-bundle', 'materialize-declaration-review', 'evidence-review-next-prompt']`.

Current command IDs: `['build-evidence-bundle', 'materialize-declaration-review']`.
...
Effort depends on upstream availability: `['L09', 'L11', 'L12']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
Ready taskcards from metadata: `['TC-EV-001']`.

Primary next task: `TC-EV-001`.
...
layer_session_handoff:
  ...
  health: "HEALTHY"
  next_task_id: "TC-EV-001"
  next_action: "Require provenance_chain for product/source declarations and link evidence packets to layer taskcards."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-EV-001`.
...
Layer-specific next action: **Require provenance_chain for product/source declarations and link evidence packets to layer taskcards.**
