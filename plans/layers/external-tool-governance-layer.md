...
layer_metadata:
  ...
  next_action: Audit .vscode/mcp.json; inventory MCP server dependencies; define governance
    contract
```

## 1. Layer Metadata

This plan is the canonical working plan for **External Tool Governance Layer** (`L22`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns external tools, MCP servers, IDE integrations, API dependencies, credentials boundaries, and allowed use contracts. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- .vscode/mcp.json
- external model/tool endpoints
- credential use policies
- tool dependency inventory

## 4. Explicit Non-Scope

- untracked tool activation
- secret exposure

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for MCP and external tool governance.
...
`INVENTORY`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- MCP is active but not formally governed.
- External LLM cascade and llm.professionalize.com supervision need declared contracts.
- Tool outputs must be evidence, not unquestioned authority.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-EXT-001"
  next_action: "Inventory MCP/external tools and create allowed-use, credential, and evidence contracts."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-EXT-001`.
...
Layer-specific next action: **Inventory MCP/external tools and create allowed-use, credential, and evidence contracts.**
