...
layer_metadata:
  ...
  maturity_current: 2
  maturity_target: 4
  current_stage: PROVENANCE_BACKFILL
  current_owner: null
...
This plan is the canonical working plan for **Provenance and Artifact Identity Layer** (`L26`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns stable identity of specs, artifacts, generated code, evidence, fixtures, packages, and claims. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.
...
- claim IDs and fact IDs

## 4. Explicit Non-Scope

- anonymous generated outputs
- untraceable manual edits

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for artifact identity and lineage.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `PROVENANCE_BACKFILL`.
- Maturity: `2/4`.
- Existing evidence paths: `[]`.

## 10. Current Execution Stage

`PROVENANCE_BACKFILL`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- SAL found invalid fact ID issues and added registry repairs.
- Generated artifacts must identify their source facts, prompts/skills, and validation evidence.
- New uploads/copies may look fresh; content provenance must be authoritative.
...
Current skill IDs: `['build-evidence-bundle']`.

Current command IDs: `['build-evidence-bundle']`.
...
Effort depends on upstream availability: `['L08']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-PROV-001"
  next_action: "Backfill artifact identity fields into evidence, sample manifests, and generated source metadata."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-PROV-001`.
...
Layer-specific next action: **Backfill artifact identity fields into evidence, sample manifests, and generated source metadata.**
