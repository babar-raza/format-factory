...
layer_metadata:
  ...
  next_action: "Audit samples/by-format/ \u2014 verify coverage for all 24 formats;\
    \ add governance"
```

## 1. Layer Metadata

This plan is the canonical working plan for **Sample Corpus Layer** (`L04`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns sample files and fixtures used for parsing, roundtrip, oracle comparison, negative controls, and regression compatibility. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- samples/by-format fixtures
- generated and legally usable sample corpus
- fixture manifests and checksums
- positive, negative, edge-case and preservation samples

## 4. Explicit Non-Scope

- product implementation
- external package release

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for sample corpus and fixture authority.
...
`COVERAGE_AUDIT`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- Existing memory notes roughly 177 samples but no complete governance contract.
...
Ready taskcards from metadata: `['TC-CORP-001']`.

Primary next task: `TC-CORP-001`.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-CORP-001"
  next_action: "Inventory sample coverage across all active formats and create manifest entries with legal/source/provenance/checksum metadata."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-CORP-001`.
...
Layer-specific next action: **Inventory sample coverage across all active formats and create manifest entries with legal/source/provenance/checksum metadata.**
