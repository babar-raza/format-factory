...
layer_metadata:
  ...
  next_action: Code-enforce lane ownership and DAG ordering; call overclaim detector
    on every cycle
  handoff_id: null
```
...
This plan is the canonical working plan for **Supervisor Sprint Layer** (`L11`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns the autonomous sprint loop: coordinator lane, work lanes, audit lanes, rework loops, gates, and terminal verdicts. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- autonomous-loop and post-sprint-loop commands
- multi-train execution layout
- taskcard-driven state machine
- acceptance, rework, blocked-external verdicts

## 4. Explicit Non-Scope

- skipping skills
- manual source mutations outside governance

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for autonomous sprint orchestration.
...
- Upstream layers: `['L08', 'L09', 'L10', 'L12', 'L13']`.
...
- Status: `GOVERNED_OPERATIONAL`.
- Health: `HEALTHY`.
- Stage: `GOVERNED_OPERATION`.
- Maturity: `5/5`.
- Existing evidence paths: `['reports/supervisor/session-resume.md', 'reports/supervisor/next-sprint.md', '.local/supervisor/continuation-signal.json']`.

## 10. Current Execution Stage

`GOVERNED_OPERATION`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.

## 11. Current Maturity Assessment

Maturity is currently **5**. This means the layer has enough structure to guide work, but it still needs stronger proof, backfill, automation, or registry enforcement before it can be treated as fully production-grade.
...
- User wants unlimited/continuing iterations and parallel trains instead of narrow one-shot sprints.
- Human review should happen only after agent verification and when external authority is actually needed.
- Supervisor must prevent plan execution from drifting into unrelated product hardening.
...
Current skill IDs: `['autonomous-loop', 'post-sprint-loop', 'post-sprint-audit']`.

Current command IDs: `['autonomous-loop', 'post-sprint-loop', 'post-sprint-audit']`.
...
Effort depends on upstream availability: `['L08', 'L09', 'L10', 'L12', 'L13']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
Ready taskcards from metadata: `['TC-SUP-001', 'TC-SUP-002']`.

Primary next task: `TC-SUP-001`.
...
layer_session_handoff:
  ...
  health: "HEALTHY"
  next_task_id: "TC-SUP-001"
  next_action: "Enforce coordinator plus product/healing/verification lanes with 1-2 validation repair loops before final verdict."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-SUP-001`.
...
Layer-specific next action: **Enforce coordinator plus product/healing/verification lanes with 1-2 validation repair loops before final verdict.**
