...
layer_metadata:
  ...
  maturity_current: 2
  maturity_target: 5
  current_stage: DESIGN_AND_BACKFILL
  current_owner: null
  ...
  next_action: Define per-format language obligation registry; link V82 oracle obligations
    to this layer
```

## 1. Layer Metadata

This plan is the canonical working plan for **Format Language Obligation Layer** (`L27`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer translates SAL facts into format-language obligations: elements, attributes, records, sections, containers, binary fields, constraints, and preservation duties. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- obligation ledgers per format
- required/optional/deprecated element classification
- syntax, semantic, preservation, and roundtrip duties
- gap ledgers for unsupported obligations
...
- Defines the contracts, registries, evidence, and acceptance criteria for format grammar and obligation extraction.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `DESIGN_AND_BACKFILL`.
- Maturity: `2/5`.
- Existing evidence paths: `[]`.

## 10. Current Execution Stage

`DESIGN_AND_BACKFILL`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- The project needs a durable middle layer between raw SAL facts and capability claims.
- DOM-level and QName-level gap analysis is not yet uniformly documented for every product.
- Obligations must distinguish full commercial targets from reduced/FOSS subset targets.
...
Current skill IDs: `['build-obligation-register', 'update-obligation-entry', 'verify-obligation-entry']`.

Current command IDs: `['build-obligation-register', 'update-obligation-entry', 'verify-obligation-entry']`.
...
Effort depends on upstream availability: `['L01', 'L05']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-OBL-001"
  next_action: "Create obligation ledgers for the commercial .NET target formats and reduced/FOSS pilot formats, then link each obligation to SAL facts and capability records."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-OBL-001`.
...
Layer-specific next action: **Create obligation ledgers for the commercial .NET target formats and reduced/FOSS pilot formats, then link each obligation to SAL facts and capability records.**
