...
layer_metadata:
  ...
  next_action: Consolidate 6 hardening addenda into canonical plan files; create plans/layers/
    as permanent authority
```

## 1. Layer Metadata

This plan is the canonical working plan for **Plan and Prompt Authority Layer** (`L10`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns reusable prompts, plan hardening prompts, authority order, and anti-bypass rules. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- prompt/autonomous plans
- plan hardening and execution prompts
- authority hierarchy and stop rules
- Espanso-ready reusable instructions

## 4. Explicit Non-Scope

- direct source edits without a plan/taskcard

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for prompt authority and plan governance.
...
- Maturity: `4/5`.
- Existing evidence paths: `['plans/master-plan.md', 'plans/strategic/spec-to-feature-radical-correction-plan.md']`.
...
- The user repeatedly requires complete reusable Espanso prompts with no omissions.
- Plans must be hardened before execution and cannot be advisory-only.
- Multiple challenger plans must be unified into one normalized execution plan when present.
...
Current skill IDs: `['plan-hardening', 'build-context-pack', 'export-plan-context']`.

Current command IDs: `['plan-hardening', 'build-context-pack', 'export-plan-context']`.
...
Ready taskcards from metadata: `['TC-PLAN-001']`.

Primary next task: `TC-PLAN-001`.
...
layer_session_handoff:
  ...
  health: "HEALTHY"
  next_task_id: "TC-PLAN-001"
  next_action: "Keep plan/harden/execute/audit/expand prompt loop aligned with layer control-plane and taskcards."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-PLAN-001`.
...
Layer-specific next action: **Keep plan/harden/execute/audit/expand prompt loop aligned with layer control-plane and taskcards.**
