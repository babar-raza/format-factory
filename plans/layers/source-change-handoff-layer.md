...
layer_metadata:
  ...
  next_action: Define source ownership map; enforce single mutation owner per file
    path (SUP-GAP-006)
```

## 1. Layer Metadata

This plan is the canonical working plan for **Source Change Handoff Layer** (`L15`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns who may change which paths, how multiple agents avoid collisions, and how source changes are handed off between lanes. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- source ownership maps
- coordinator-owned integration files
- branch/state/change ledgers
- handoff records for partial work

## 4. Explicit Non-Scope

- uncoordinated parallel edits
- manual merge guesswork

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for source mutation ownership and handoff.
...
`CONTRACT_DESIGN`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- Multiple trains require explicit path locks and handoff contracts.
- Shared files need coordinator ownership and lane-level patch queues.
- Existing prompts mention ownership, but registry enforcement is incomplete.

| gap_id | severity | current_state | target_state | task_id | source |
|---|---|---|---|---|---|
| SCH-GAP-001 | HIGH | `skill_ids: []` in `plans/layers/index.yaml` (L15) — zero registered skill defines or enforces a source ownership map, path locks, or handoff records between lanes. | A registered skill exists that defines the source ownership map and enforces a single mutation owner per file path, producing handoff records for partial work (feeding TC-SCH-001). | null | Zero skill coverage confirmed via plans/layers/index.yaml — skill_ids: [] and no matching product_track in .supervisor/skill-registry.yaml. Registered via TC-EXT-008 (external skill adoption plan, Finding 3). |
...
- 2026-07-14: Registered gap `SCH-GAP-001` via `/register-layer-gap` (TC-EXT-008-02, plan `plans/.claude/yes-my-earlier-answer-humming-waffle.md`, Finding 3). See §14.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-SCH-001"
  next_action: "Create source ownership map and enforce single mutation owner per path with handoff records."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-SCH-001`.
...
Layer-specific next action: **Create source ownership map and enforce single mutation owner per path with handoff records.**
