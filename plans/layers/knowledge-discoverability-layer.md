...
layer_metadata:
  ...
  maturity_current: 2
  maturity_target: 4
  current_stage: DISCOVERY_HARDENING
  current_owner: null
  ...
  next_action: Audit MEMORY.md and project-memory.md (591KB); define structured discoverability
    contracts
```

## 1. Layer Metadata

This plan is the canonical working plan for **Knowledge Discoverability Layer** (`L23`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns where agents find repository knowledge, layer plans, conventions, learned audits, and reusable summaries before work starts. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- plans/layers index
- docs/governance summaries
- README/AGENTS pointers
- discoverability reports

## 4. Explicit Non-Scope

- tribal knowledge hidden in chat only

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for making system knowledge findable.
...
- Status: `HARDENING_REQUIRED`.
- Health: `DEGRADED`.
- Stage: `DISCOVERY_HARDENING`.
- Maturity: `2/4`.
- Existing evidence paths: `[]`.

## 10. Current Execution Stage

`DISCOVERY_HARDENING`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- User noted agents get confused when new work requires theme/layout/content knowledge.
- Information may exist but agents do not reliably find it.
- Layer plans must summarize and reference key files so future agents can start correctly.
...
Current skill IDs: `['sync-memory', 'build-context-pack']`.

Current command IDs: `['sync-memory', 'build-context-pack']`.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-KNOW-001"
  next_action: "Add findable layer summaries, key paths, and discovery rules for each subsystem."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-KNOW-001`.
...
Layer-specific next action: **Add findable layer summaries, key paths, and discovery rules for each subsystem.**
