...
layer_metadata:
  ...
  maturity_current: 2
  maturity_target: 4
  current_stage: RELEASE_HARDENING
  current_owner: null
  ...
  next_action: 'BLOCKED_EXTERNAL: Gate 11 execution requires Babar Raza approval before
    PyPI/NuGet release'
```

## 1. Layer Metadata

This plan is the canonical working plan for **Package Release Layer** (`L18`). It replaces placeholder/stub prose with a governed layer contract, current known state, gaps, and executable next actions based on the Format Factory project memory and the existing layer-plan pattern.

## 2. Authority and Purpose

This layer owns package manager publication, CI release workflows, credentials, versioning, icons/metadata, and release evidence. Its authority is limited to its owned scope and must be exercised through registered skills, taskcards, evidence declarations, and validation gates.

## 3. Scope

- NuGet/PyPI/npm/Maven workflows
- release secrets and external gates
- package metadata and icons
- local runner and remote Actions verification

## 4. Explicit Non-Scope

- publishing without external credentials/approval
- release claims without artifacts

## 5. Owned Decisions

- Defines the contracts, registries, evidence, and acceptance criteria for package publication and release workflows.
...
`RELEASE_HARDENING`. Work may proceed only after skill coverage is checked. If no skill covers a required action, the agent must write a skill-gap report and stop the uncovered portion while continuing any covered work.
...
- NuGet asposefoss has several products but some missed icons in first release.
- PyPI org remains pending; Maven Central workflow has a Java pilot.
- Local runner/act distinction must be clear and repeatable.
...
Current skill IDs: `['package-install-proof', 'check-release-boundary', 'check-gate']`.

Current command IDs: `['package-install-proof', 'check-release-boundary', 'check-gate']`.
...
Effort depends on upstream availability: `['L06', 'L17', 'L21']`. When the layer has no listed dependencies, it still depends on repository governance, skill coverage, and clean working-tree preflight.
...
layer_session_handoff:
  ...
  health: "DEGRADED"
  next_task_id: "TC-PKG-001"
  next_action: "Standardize package metadata and release checklists for NuGet, Maven, PyPI, and npm with true external gates marked."
```

## 37. Exact Next Actions

1. Run skill coverage check for this layer.
2. Open/create the next taskcard `TC-PKG-001`.
...
Layer-specific next action: **Standardize package metadata and release checklists for NuGet, Maven, PyPI, and npm with true external gates marked.**
