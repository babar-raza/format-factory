"""Tests for tsv_to_sylk dogfood export."""
...
class TestTsvToSylkBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = tsv_to_sylk(MINIMAL_TSV, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.slk"
        tsv_to_sylk(MINIMAL_TSV, dest)
        assert dest.exists()
...
class TestTsvToSylkPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = tsv_to_sylk(str(MINIMAL_TSV), str(dest))
