"""Tests for tsv_to_dif dogfood export."""
...
class TestTsvToDifBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = tsv_to_dif(MINIMAL_TSV, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.dif"
        tsv_to_dif(MINIMAL_TSV, dest)
        assert dest.exists()
...
class TestTsvToDifPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = tsv_to_dif(str(MINIMAL_TSV), str(dest))
