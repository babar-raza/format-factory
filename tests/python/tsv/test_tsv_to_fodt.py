"""
Tests for tsv_to_fodt dogfood export.

Verifies that TSV rows are converted to FODT paragraphs using
Format Factory's TSV parser and FODT writer libraries.
"""
...
from tsv.tsv_to_fodt import tsv_to_fodt


class TestTsvToFodtBasic:
    """Basic conversion tests."""

    def test_returns_row_count(self, tmp_path: Path) -> None:
        """Returns an integer count of paragraphs written."""
        dest = tmp_path / "out.fodt"
        count = tsv_to_fodt(MINIMAL_TSV, dest)
...
class TestTsvToFodtContent:
    """Content correctness tests."""

    def test_produces_paragraphs(self, tmp_path: Path) -> None:
        """TSV rows appear as FODT paragraphs."""
        dest = tmp_path / "out.fodt"
        count = tsv_to_fodt(MINIMAL_TSV, dest)
        assert count >= 1
...
class TestTsvToFodtOptions:
    ...
    def test_include_header(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.fodt"
        count = tsv_to_fodt(MINIMAL_TSV, dest, include_header=True)
        assert count >= 1

    def test_no_header(self, tmp_path: Path) -> None:
        """include_header=False skips header paragraph."""
        dest = tmp_path / "out.fodt"
        count = tsv_to_fodt(MINIMAL_TSV, dest, include_header=False)
        assert count >= 0


class TestTsvToFodtPaths:
    """Path handling tests."""
    ...
    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.fodt"
        count = tsv_to_fodt(str(MINIMAL_TSV), str(dest))
        assert isinstance(count, int)
        assert dest.exists()
