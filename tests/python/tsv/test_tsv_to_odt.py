"""
Tests for tsv_to_odt dogfood export.

Verifies that TSV rows are converted to ODT paragraphs using
Format Factory's TSV parser and ODT writer libraries.
"""
...
from tsv.tsv_to_odt import tsv_to_odt


class TestTsvToOdtBasic:
    """Basic conversion tests."""

    def test_returns_row_count(self, tmp_path: Path) -> None:
        """Returns an integer count of data rows written."""
        dest = tmp_path / "out.odt"
        count = tsv_to_odt(MINIMAL_TSV, dest)
...
class TestTsvToOdtContent:
    ...
    def test_tsv_values_in_content(self, tmp_path: Path) -> None:
    ...
    def test_multi_column_converts(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = tsv_to_odt(MULTI_TSV, dest)
        assert dest.exists()
        assert count >= 0
...
class TestTsvToOdtOptions:
    ...
    def test_header_as_heading(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = tsv_to_odt(MINIMAL_TSV, dest, include_header_as_heading=True)
        assert dest.exists()
        assert count >= 0


class TestTsvToOdtPaths:
    """Path handling tests."""
    ...
    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = tsv_to_odt(str(MINIMAL_TSV), str(dest))
        assert isinstance(count, int)
        assert dest.exists()
