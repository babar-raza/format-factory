"""Sprint 334: XCF analytics — two new composite functions."""
...
class TestXcfFileSizeMod53Times800PlusImageTypeTimes1100PlusWidthTimes100PlusHeightTimes70:
    def _fn(self):
        from src.python.xcf import xcf_file_size_mod_53_times_800_plus_image_type_times_1100_plus_width_times_100_plus_height_times_70
        return xcf_file_size_mod_53_times_800_plus_image_type_times_1100_plus_width_times_100_plus_height_times_70

    def test_blue(self): assert self._fn()(BLUE) == 15370
    def test_red(self): assert self._fn()(RED) == 14570
    def test_gray(self): assert self._fn()(GRAY) == 16640
    def test_returns_int(self): assert isinstance(self._fn()(BLUE), int)
    def test_distinct_values(self):
        fn = self._fn(); vals = {fn(BLUE), fn(RED), fn(GRAY)}; assert len(vals) == 3
    def test_nonnegative(self):
        fn = self._fn()
        for p in [BLUE, RED, GRAY]: assert fn(p) >= 0
    def test_path_object_accepted(self): assert self._fn()(Path(BLUE)) == 15370
    def test_string_path_accepted(self): assert self._fn()(str(BLUE)) == 15370
    def test_gray_largest(self):
        fn = self._fn(); assert fn(GRAY) > fn(BLUE) > fn(RED)
    def test_red_smallest(self):
        fn = self._fn(); assert fn(RED) == min(fn(BLUE), fn(RED), fn(GRAY))
