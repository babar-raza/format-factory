"""Tests for toml_to_ppm dogfood export."""
...
from src.python.toml.toml_to_ppm import toml_to_ppm


class TestTOMLToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = toml_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        toml_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestTOMLToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = toml_to_ppm(str(SAMPLE), str(dest))
