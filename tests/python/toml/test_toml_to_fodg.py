"""Tests for toml_to_fodg dogfood export."""
...
from src.python.toml.toml_to_fodg import toml_to_fodg


class TestTomlToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = toml_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        toml_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestTomlToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = toml_to_fodg(str(SAMPLE), str(dest))
