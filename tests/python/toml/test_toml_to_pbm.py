"""Tests for toml_to_pbm dogfood export."""
...
from src.python.toml.toml_to_pbm import toml_to_pbm


class TestTOMLToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = toml_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        toml_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestTOMLToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = toml_to_pbm(str(SAMPLE), str(dest))
