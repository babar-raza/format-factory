"""Tests for toml_to_dif dogfood export."""
...
from src.python.toml.toml_to_dif import toml_to_dif


class TestTomlToDifBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = toml_to_dif(MINIMAL_TOML, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.dif"
        toml_to_dif(MINIMAL_TOML, dest)
        assert dest.exists()
...
class TestTomlToDifPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = toml_to_dif(str(MINIMAL_TOML), str(dest))
