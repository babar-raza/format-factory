"""Tests for abw_to_dif dogfood export."""
...
from src.python.abw.abw_to_dif import abw_to_dif


class TestAbwToDifBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = abw_to_dif(MINIMAL_ABW, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.dif"
        abw_to_dif(MINIMAL_ABW, dest)
        assert dest.exists()
...
class TestAbwToDifPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = abw_to_dif(str(MINIMAL_ABW), str(dest))
