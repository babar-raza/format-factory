"""Tests for odt_to_fodg dogfood export."""
...
from src.python.odt.odt_to_fodg import odt_to_fodg


class TestODTToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = odt_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        odt_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestODTToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = odt_to_fodg(str(SAMPLE), str(dest))
