"""Tests for odt_to_ppm dogfood export."""
...
from src.python.odt.odt_to_ppm import odt_to_ppm


class TestODTToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = odt_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        odt_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestODTToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = odt_to_ppm(str(SAMPLE), str(dest))
