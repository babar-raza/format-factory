"""Tests for odt_to_sylk dogfood export."""
...
from src.python.odt.odt_to_sylk import odt_to_sylk


class TestOdtToSylkBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = odt_to_sylk(MINIMAL_ODT, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.slk"
        odt_to_sylk(MINIMAL_ODT, dest)
        assert dest.exists()
...
class TestOdtToSylkPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = odt_to_sylk(str(MINIMAL_ODT), str(dest))
