"""Tests for odt_to_pgm dogfood export."""
...
from src.python.odt.odt_to_pgm import odt_to_pgm


class TestODTToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = odt_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        odt_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestODTToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = odt_to_pgm(str(SAMPLE), str(dest))
