"""Tests for odt_to_pbm dogfood export."""
...
from src.python.odt.odt_to_pbm import odt_to_pbm


class TestODTToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = odt_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        odt_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestODTToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = odt_to_pbm(str(SAMPLE), str(dest))
