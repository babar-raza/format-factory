"""Tests for fodp_to_fodg dogfood export."""
...
from src.python.fodp.fodp_to_fodg import fodp_to_fodg


class TestFodpToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = fodp_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        fodp_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestFodpToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = fodp_to_fodg(str(SAMPLE), str(dest))
