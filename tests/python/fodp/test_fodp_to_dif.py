"""Tests for fodp_to_dif dogfood export."""
...
from src.python.fodp.fodp_to_dif import fodp_to_dif


class TestFodpToDifBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = fodp_to_dif(MINIMAL_FODP, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.dif"
        fodp_to_dif(MINIMAL_FODP, dest)
        assert dest.exists()
...
class TestFodpToDifPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = fodp_to_dif(str(MINIMAL_FODP), str(dest))
