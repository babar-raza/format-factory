"""Tests for fodp_to_pbm dogfood export."""
...
from src.python.fodp.fodp_to_pbm import fodp_to_pbm


class TestFODPToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = fodp_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        fodp_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODPToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = fodp_to_pbm(str(SAMPLE), str(dest))
