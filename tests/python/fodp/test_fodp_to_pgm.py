"""Tests for fodp_to_pgm dogfood export."""
...
from src.python.fodp.fodp_to_pgm import fodp_to_pgm


class TestFODPToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fodp_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        fodp_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODPToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fodp_to_pgm(str(SAMPLE), str(dest))
