"""Tests for fodp_to_sylk dogfood export."""
...
from src.python.fodp.fodp_to_sylk import fodp_to_sylk


class TestFodpToSylkBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = fodp_to_sylk(MINIMAL_FODP, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.slk"
        fodp_to_sylk(MINIMAL_FODP, dest)
        assert dest.exists()
...
class TestFodpToSylkPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = fodp_to_sylk(str(MINIMAL_FODP), str(dest))
