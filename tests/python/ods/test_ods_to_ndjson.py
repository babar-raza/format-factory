"""
Tests for ods_to_ndjson dogfood export.

Verifies that ODS spreadsheet rows are converted to NDJSON records using
Format Factory's ODS parser and NDJSON writer libraries.
"""
...
SINGLE_ODS = _REPO / "samples" / "by-format" / "ods" / "valid" / "single-cell.ods"

from ods.ods_to_ndjson import ods_to_ndjson


class TestOdsToNdjsonBasic:
    ...
    def test_returns_record_count(self, tmp_path: Path) -> None:
        ...
        count = ods_to_ndjson(MINIMAL_ODS, dest)
    ...
    def test_output_file_created(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.ndjson"
        ods_to_ndjson(MINIMAL_ODS, dest)
        assert dest.exists()
...
class TestOdsToNdjsonHeaders:
    ...
    def test_first_row_becomes_keys(self, tmp_path: Path) -> None:
        ...
        # minimal-spreadsheet.ods: header row = Name, Value
    ...
    def test_header_row_not_included_as_record(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.ndjson"
        count = ods_to_ndjson(MINIMAL_ODS, dest)
        # minimal-spreadsheet.ods: 2 rows total (1 header + 1 data) → 1 record
        assert count == 1
...
class TestOdsToNdjsonRowIndex:
    ...
    def test_row_index_included_when_enabled(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.ndjson"
        ods_to_ndjson(MINIMAL_ODS, dest, include_row_index=True)
        lines = [l for l in dest.read_text(encoding="utf-8").splitlines() if l.strip()]
        for line in lines:
    ...
    def test_row_index_sequential(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.ndjson"
        count = ods_to_ndjson(MINIMAL_ODS, dest, include_row_index=True)
        lines = [l for l in dest.read_text(encoding="utf-8").splitlines() if l.strip()]
        indices = [json.loads(l)["row_index"] for l in lines]
        assert indices == list(range(count))


class TestOdsToNdjsonPaths:
    """Path handling tests."""
    ...
    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.ndjson"
        count = ods_to_ndjson(str(MINIMAL_ODS), str(dest))
        assert isinstance(count, int)
        assert dest.exists()
