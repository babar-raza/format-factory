"""Tests for ods_to_pgm dogfood export."""
...
from src.python.ods.ods_to_pgm import ods_to_pgm


class TestODSToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = ods_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        ods_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestODSToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = ods_to_pgm(str(SAMPLE), str(dest))
