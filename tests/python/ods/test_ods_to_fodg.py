"""Tests for ods_to_fodg dogfood export."""
...
from src.python.ods.ods_to_fodg import ods_to_fodg


class TestODSToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = ods_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        ods_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestODSToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = ods_to_fodg(str(SAMPLE), str(dest))
