"""
Tests for ods_to_fodt dogfood export.

Verifies that ODS rows are converted to FODT paragraphs using
Format Factory's ODS parser and FODT writer libraries.
"""
...
from ods.ods_to_fodt import ods_to_fodt


class TestOdsToFodtBasic:
    """Basic conversion tests."""

    def test_returns_row_count(self, tmp_path: Path) -> None:
        """Returns an integer count of paragraphs written."""
        dest = tmp_path / "out.fodt"
        count = ods_to_fodt(MINIMAL_ODS, dest)
...
class TestOdsToFodtContent:
    """Content correctness tests."""

    def test_produces_paragraphs(self, tmp_path: Path) -> None:
        """ODS rows appear as FODT paragraphs."""
        dest = tmp_path / "out.fodt"
        count = ods_to_fodt(MINIMAL_ODS, dest)
        assert count >= 1
...
class TestOdsToFodtPaths:
    ...
    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.fodt"
        count = ods_to_fodt(str(MINIMAL_ODS), str(dest))
        assert isinstance(count, int)
        assert dest.exists()
