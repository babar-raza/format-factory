"""Tests for ods_to_pbm dogfood export."""
...
from src.python.ods.ods_to_pbm import ods_to_pbm


class TestODSToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = ods_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        ods_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestODSToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = ods_to_pbm(str(SAMPLE), str(dest))
