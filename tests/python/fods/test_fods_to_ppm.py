"""Tests for fods_to_ppm dogfood export."""
...
from src.python.fods.fods_to_ppm import fods_to_ppm


class TestFODSToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = fods_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        fods_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODSToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = fods_to_ppm(str(SAMPLE), str(dest))
