"""Tests for fods_to_pbm dogfood export."""
...
from src.python.fods.fods_to_pbm import fods_to_pbm


class TestFODSToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = fods_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        fods_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODSToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = fods_to_pbm(str(SAMPLE), str(dest))
