"""Tests for fods_to_pgm dogfood export."""
...
from src.python.fods.fods_to_pgm import fods_to_pgm


class TestFODSToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fods_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        fods_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODSToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fods_to_pgm(str(SAMPLE), str(dest))
