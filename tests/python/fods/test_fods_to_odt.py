"""
Tests for fods_to_odt dogfood export.

Verifies that FODS rows are converted to ODT paragraphs using
Format Factory's FODS parser and ODT writer libraries.
"""
...
from fods.fods_to_odt import fods_to_odt


class TestFodsToOdtBasic:
    """Basic conversion tests."""

    def test_returns_row_count(self, tmp_path: Path) -> None:
        """Returns an integer count of paragraphs written."""
        dest = tmp_path / "out.odt"
        count = fods_to_odt(MINIMAL_FODS, dest)
...
class TestFodsToOdtContent:
    ...
    def test_produces_paragraphs(self, tmp_path: Path) -> None:
        """FODS rows produce ODT paragraphs."""
        dest = tmp_path / "out.odt"
        count = fods_to_odt(MINIMAL_FODS, dest)
        assert count >= 1
    ...
    def test_multi_sheet_converts(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = fods_to_odt(MULTI_FODS, dest)
        assert dest.exists()
        assert count >= 0
...
class TestFodsToOdtOptions:
    ...
    def test_skip_empty_rows_default(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = fods_to_odt(MINIMAL_FODS, dest, skip_empty_rows=True)
        assert count >= 0

    def test_sheet_index_default(self, tmp_path: Path) -> None:
        """Default sheet_index=0 exports the first sheet."""
        dest = tmp_path / "out.odt"
        count = fods_to_odt(MINIMAL_FODS, dest, sheet_index=0)
        assert count >= 0


class TestFodsToOdtPaths:
    """Path handling tests."""
    ...
    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = fods_to_odt(str(MINIMAL_FODS), str(dest))
        assert isinstance(count, int)
        assert dest.exists()
