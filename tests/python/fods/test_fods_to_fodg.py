"""Tests for fods_to_fodg dogfood export."""
...
from src.python.fods.fods_to_fodg import fods_to_fodg


class TestFodsToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = fods_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        fods_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestFodsToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = fods_to_fodg(str(SAMPLE), str(dest))
