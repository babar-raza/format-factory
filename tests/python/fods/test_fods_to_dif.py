"""Tests for fods_to_dif dogfood export."""
...
from src.python.fods.fods_to_dif import fods_to_dif


class TestFodsToDifBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = fods_to_dif(MINIMAL_FODS, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.dif"
        fods_to_dif(MINIMAL_FODS, dest)
        assert dest.exists()
...
class TestFodsToDifPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = fods_to_dif(str(MINIMAL_FODS), str(dest))
