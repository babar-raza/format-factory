"""Tests for fodg_to_sylk dogfood export."""
...
from src.python.fodg.fodg_to_sylk import fodg_to_sylk


class TestFodgToSylkBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = fodg_to_sylk(MINIMAL_FODG, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.slk"
        fodg_to_sylk(MINIMAL_FODG, dest)
        assert dest.exists()
...
class TestFodgToSylkPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.slk"
        count = fodg_to_sylk(str(MINIMAL_FODG), str(dest))
