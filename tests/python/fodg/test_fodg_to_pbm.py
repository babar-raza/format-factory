"""Tests for fodg_to_pbm dogfood export."""
...
from src.python.fodg.fodg_to_pbm import fodg_to_pbm


class TestFODGToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = fodg_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        fodg_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODGToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = fodg_to_pbm(str(SAMPLE), str(dest))
