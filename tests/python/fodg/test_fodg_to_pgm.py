"""Tests for fodg_to_pgm dogfood export."""
...
from src.python.fodg.fodg_to_pgm import fodg_to_pgm


class TestFODGToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fodg_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        fodg_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODGToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fodg_to_pgm(str(SAMPLE), str(dest))
