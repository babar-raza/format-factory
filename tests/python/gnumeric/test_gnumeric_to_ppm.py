"""Tests for gnumeric_to_ppm dogfood export."""
...
from src.python.gnumeric.gnumeric_to_ppm import gnumeric_to_ppm


class TestGnumericToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = gnumeric_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        gnumeric_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestGnumericToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = gnumeric_to_ppm(str(SAMPLE), str(dest))
