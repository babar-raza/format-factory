"""Tests for gnumeric_to_pbm dogfood export."""
...
from src.python.gnumeric.gnumeric_to_pbm import gnumeric_to_pbm


class TestGnumericToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = gnumeric_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        gnumeric_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestGnumericToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = gnumeric_to_pbm(str(SAMPLE), str(dest))
