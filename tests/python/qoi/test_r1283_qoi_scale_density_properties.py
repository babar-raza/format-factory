"""Tests for R1283: QoiDocument scale and density classification properties.
...
class TestPixelDensityClass:
    def test_micro_64x64(self):
        # 4096 pixels → micro (≤ 4096)
        doc = _make_doc(64, 64)
        assert doc.pixel_density_class == "micro"

    def test_micro_zero(self):
        doc = _make_doc(0, 0)
        assert doc.pixel_density_class == "micro"
    ...
    def test_boundary_4097_is_small(self):
        # 4097 pixels > 4096 threshold → small (assuming rectangle)
        doc = _make_doc(65, 64)  # 4160 pixels → small
        assert doc.pixel_density_class == "small"
...
class TestCrossPropertyConsistency:
    ...
    def test_rgba_banner(self):
        # RGBA channels work the same for scale properties
        doc = _make_doc(400, 100, channels=4)
        assert doc.is_banner is True
