"""
Tests for sylk_to_abw dogfood export.

Verifies that SYLK rows are converted to ABW paragraphs using
Format Factory's SYLK parser and ABW writer libraries.
"""
...
class TestSylkToAbwBasic:
    """Basic conversion tests."""

    def test_returns_row_count(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        count = sylk_to_abw(MINIMAL_SYLK, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        sylk_to_abw(MINIMAL_SYLK, dest)
        assert dest.exists()
...
class TestSylkToAbwPaths:
    ...
    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        count = sylk_to_abw(str(MINIMAL_SYLK), str(dest))
