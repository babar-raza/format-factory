"""
Tests for sylk_to_fodt dogfood export.

Verifies that SYLK grid rows are converted to FODT paragraphs using
Format Factory's SYLK parser and FODT writer libraries.
"""
from __future__ import annotations

import sys
from pathlib import Path

import pytest

_REPO = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(_REPO / "src" / "python"))
sys.path.insert(0, str(_REPO))

MINIMAL_SLK = _REPO / "samples" / "by-format" / "sylk" / "valid" / "minimal-2x2.slk"
NUMERIC_SLK = _REPO / "samples" / "by-format" / "sylk" / "valid" / "numeric-row.slk"

from sylk.sylk_to_fodt import sylk_to_fodt


class TestSylkToFodtBasic:
    """Basic conversion tests."""

    def test_returns_row_count(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        count = sylk_to_fodt(MINIMAL_SLK, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        sylk_to_fodt(MINIMAL_SLK, dest)
        assert dest.exists()

    def test_output_is_xml(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        sylk_to_fodt(MINIMAL_SLK, dest)
        content = dest.read_text(encoding="utf-8")
        assert "<?xml" in content or "<office:document" in content

    def test_output_contains_paragraph(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        sylk_to_fodt(MINIMAL_SLK, dest)
        content = dest.read_text(encoding="utf-8")
        assert "text:p" in content


class TestSylkToFodtContent:
    """Content correctness tests."""

    def test_produces_paragraphs(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        count = sylk_to_fodt(MINIMAL_SLK, dest)
        assert count >= 1

    def test_numeric_sylk_converts(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        count = sylk_to_fodt(NUMERIC_SLK, dest)
        assert dest.exists() and count >= 1

    def test_custom_separator(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        sylk_to_fodt(MINIMAL_SLK, dest, separator=" , ")
        content = dest.read_text(encoding="utf-8")
        assert " , " in content


class TestSylkToFodtPaths:
    """Path handling tests."""

    def test_creates_parent_directories(self, tmp_path: Path) -> None:
        dest = tmp_path / "nested" / "dir" / "out.fodt"
        sylk_to_fodt(MINIMAL_SLK, dest)
        assert dest.exists()

    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.fodt"
        count = sylk_to_fodt(str(MINIMAL_SLK), str(dest))
        assert isinstance(count, int) and dest.exists()
