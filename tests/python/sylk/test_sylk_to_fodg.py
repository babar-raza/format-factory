"""Tests for sylk_to_fodg dogfood export."""
...
from src.python.sylk.sylk_to_fodg import sylk_to_fodg


class TestSylkToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = sylk_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        sylk_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestSylkToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = sylk_to_fodg(str(SAMPLE), str(dest))
