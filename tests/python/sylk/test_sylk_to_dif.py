"""Tests for sylk_to_dif dogfood export."""
...
from src.python.sylk.sylk_to_dif import sylk_to_dif


class TestSylkToDifBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = sylk_to_dif(MINIMAL_SYLK, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.dif"
        sylk_to_dif(MINIMAL_SYLK, dest)
        assert dest.exists()
...
class TestSylkToDifPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = sylk_to_dif(str(MINIMAL_SYLK), str(dest))
