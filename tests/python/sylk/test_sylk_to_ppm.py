"""Tests for sylk_to_ppm dogfood export."""
...
from src.python.sylk.sylk_to_ppm import sylk_to_ppm


class TestSYLKToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = sylk_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        sylk_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestSYLKToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = sylk_to_ppm(str(SAMPLE), str(dest))
