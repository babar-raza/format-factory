"""Tests for sylk_to_pbm dogfood export."""
...
from src.python.sylk.sylk_to_pbm import sylk_to_pbm


class TestSYLKToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = sylk_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        sylk_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestSYLKToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = sylk_to_pbm(str(SAMPLE), str(dest))
