"""Tests for sylk_to_gnumeric dogfood export."""
...
class TestSylkToGnumericBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.gnumeric"
        count = sylk_to_gnumeric(MINIMAL_SYLK, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.gnumeric"
        sylk_to_gnumeric(MINIMAL_SYLK, dest)
        assert dest.exists()
...
class TestSylkToGnumericPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.gnumeric"
        count = sylk_to_gnumeric(str(MINIMAL_SYLK), str(dest))
