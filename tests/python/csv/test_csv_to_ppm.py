"""Tests for csv_to_ppm dogfood export."""
...
from src.python.csv.csv_to_ppm import csv_to_ppm


class TestCSVToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = csv_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        csv_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestCSVToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = csv_to_ppm(str(SAMPLE), str(dest))
