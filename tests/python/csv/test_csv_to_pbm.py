"""Tests for csv_to_pbm dogfood export."""
...
from src.python.csv.csv_to_pbm import csv_to_pbm


class TestCSVToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = csv_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        csv_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestCSVToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = csv_to_pbm(str(SAMPLE), str(dest))
