"""Tests for csv_to_pgm dogfood export."""
...
from src.python.csv.csv_to_pgm import csv_to_pgm


class TestCSVToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = csv_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        csv_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestCSVToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = csv_to_pgm(str(SAMPLE), str(dest))
