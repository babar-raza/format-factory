"""Tests for R1233: CsvDocument row count scale classification properties.
...
def _make_doc(row_count: int, col_count: int = 3, has_header: bool = True,
              jagged: bool = False) -> CsvDocument:
    """Build a CsvDocument stub with the given dimensions."""
...
class TestIsTiny:
    ...
    def test_boundary_five_is_tiny(self):
        doc = _make_doc(5)
        assert doc.is_tiny is True
...
class TestHasUniformRows:
    ...
    def test_uniform_single_column(self):
        doc = _make_doc(5, col_count=1)
        assert doc.has_uniform_rows is True
