"""Tests for fodt_to_pgm dogfood export."""
...
from src.python.fodt.fodt_to_pgm import fodt_to_pgm


class TestFODTToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fodt_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        fodt_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODTToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = fodt_to_pgm(str(SAMPLE), str(dest))
