"""Tests for fodt_to_dif dogfood export."""
...
from src.python.fodt.fodt_to_dif import fodt_to_dif


class TestFodtToDifBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = fodt_to_dif(MINIMAL_FODT, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.dif"
        fodt_to_dif(MINIMAL_FODT, dest)
        assert dest.exists()
...
class TestFodtToDifPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.dif"
        count = fodt_to_dif(str(MINIMAL_FODT), str(dest))
