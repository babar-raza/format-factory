"""Tests for fodt_to_ppm dogfood export."""
...
from src.python.fodt.fodt_to_ppm import fodt_to_ppm


class TestFODTToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = fodt_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        fodt_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestFODTToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = fodt_to_ppm(str(SAMPLE), str(dest))
