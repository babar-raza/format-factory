"""
Tests for fodt_to_abw dogfood export.

Verifies that FODT blocks are converted to ABW paragraphs using
Format Factory's FODT parser and ABW writer libraries.
"""
from __future__ import annotations

import sys
from pathlib import Path

import pytest

_REPO = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(_REPO / "src" / "python"))
sys.path.insert(0, str(_REPO))

MINIMAL_FODT = _REPO / "samples" / "by-format" / "fodt" / "minimal-document.fodt"

from fodt.fodt_to_abw import fodt_to_abw


class TestFodtToAbwBasic:
    """Basic conversion tests."""

    def test_returns_block_count(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        count = fodt_to_abw(MINIMAL_FODT, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        fodt_to_abw(MINIMAL_FODT, dest)
        assert dest.exists()

    def test_produces_blocks(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        count = fodt_to_abw(MINIMAL_FODT, dest)
        assert count >= 1

    def test_output_is_xml(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        fodt_to_abw(MINIMAL_FODT, dest)
        content = dest.read_text(encoding="utf-8")
        assert "<abiword" in content or "<?xml" in content


class TestFodtToAbwPaths:
    """Path handling tests."""

    def test_creates_parent_directories(self, tmp_path: Path) -> None:
        dest = tmp_path / "nested" / "dir" / "out.abw"
        fodt_to_abw(MINIMAL_FODT, dest)
        assert dest.exists()

    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        dest = tmp_path / "out.abw"
        count = fodt_to_abw(str(MINIMAL_FODT), str(dest))
        assert isinstance(count, int) and dest.exists()
