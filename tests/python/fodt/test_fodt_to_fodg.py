"""Tests for fodt_to_fodg dogfood export."""
...
from src.python.fodt.fodt_to_fodg import fodt_to_fodg


class TestFodtToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = fodt_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        fodt_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestFodtToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = fodt_to_fodg(str(SAMPLE), str(dest))
