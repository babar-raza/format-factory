"""Tests for dif_to_pgm dogfood export."""
...
from src.python.dif.dif_to_pgm import dif_to_pgm


class TestDIFToPGMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = dif_to_pgm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pgm"
        dif_to_pgm(SAMPLE, dest)
        assert dest.exists()
...
class TestDIFToPGMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pgm"
        count = dif_to_pgm(str(SAMPLE), str(dest))
