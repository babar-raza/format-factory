"""Tests for dif_to_gnumeric dogfood export."""
...
from dif.dif_to_gnumeric import dif_to_gnumeric


class TestDifToGnumericBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.gnumeric"
        count = dif_to_gnumeric(MINIMAL_DIF, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.gnumeric"
        dif_to_gnumeric(MINIMAL_DIF, dest)
        assert dest.exists()
...
class TestDifToGnumericPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.gnumeric"
        count = dif_to_gnumeric(str(MINIMAL_DIF), str(dest))
