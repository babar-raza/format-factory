"""
Tests for dif_to_odt dogfood export.

Verifies that DIF rows are converted to ODT paragraphs using
Format Factory's DIF parser and ODT writer libraries.
"""
...
from dif.dif_to_odt import dif_to_odt


class TestDifToOdtBasic:
    """Basic conversion tests."""

    def test_returns_row_count(self, tmp_path: Path) -> None:
        """Returns an integer count of paragraphs written."""
        dest = tmp_path / "out.odt"
        count = dif_to_odt(MINIMAL_DIF, dest)
...
class TestDifToOdtContent:
    ...
    def test_produces_paragraphs(self, tmp_path: Path) -> None:
        """DIF rows produce ODT paragraphs."""
        dest = tmp_path / "out.odt"
        count = dif_to_odt(MINIMAL_DIF, dest)
        assert count >= 1
    ...
    def test_numeric_row_converts(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = dif_to_odt(NUMERIC_DIF, dest)
        assert dest.exists()
        assert count >= 1
...
class TestDifToOdtOptions:
    """Option flag tests."""

    def test_custom_separator(self, tmp_path: Path) -> None:
        """Custom separator joins values in paragraphs."""
        dest = tmp_path / "out.odt"
        dif_to_odt(MINIMAL_DIF, dest, separator=" | ")
        with zipfile.ZipFile(dest) as z:
            content = z.read("content.xml").decode("utf-8")
        assert " | " in content

    def test_skip_empty_rows_default(self, tmp_path: Path) -> None:
        """skip_empty_rows=True (default) skips all-empty rows."""
        dest = tmp_path / "out.odt"
        count = dif_to_odt(MINIMAL_DIF, dest, skip_empty_rows=True)
        assert count >= 0


class TestDifToOdtPaths:
    """Path handling tests."""
    ...
    def test_accepts_string_paths(self, tmp_path: Path) -> None:
        ...
        dest = tmp_path / "out.odt"
        count = dif_to_odt(str(MINIMAL_DIF), str(dest))
        assert isinstance(count, int)
        assert dest.exists()
