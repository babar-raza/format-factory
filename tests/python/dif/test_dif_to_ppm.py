"""Tests for dif_to_ppm dogfood export."""
...
from src.python.dif.dif_to_ppm import dif_to_ppm


class TestDIFToPPMBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = dif_to_ppm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.ppm"
        dif_to_ppm(SAMPLE, dest)
        assert dest.exists()
...
class TestDIFToPPMPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.ppm"
        count = dif_to_ppm(str(SAMPLE), str(dest))
