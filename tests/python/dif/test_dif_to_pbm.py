"""Tests for dif_to_pbm dogfood export."""
...
from src.python.dif.dif_to_pbm import dif_to_pbm


class TestDIFToPbmBasic:
    def test_returns_row_count(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = dif_to_pbm(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.pbm"
        dif_to_pbm(SAMPLE, dest)
        assert dest.exists()
...
class TestDIFToPbmPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.pbm"
        count = dif_to_pbm(str(SAMPLE), str(dest))
