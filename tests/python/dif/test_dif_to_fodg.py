"""Tests for dif_to_fodg dogfood export."""
...
from src.python.dif.dif_to_fodg import dif_to_fodg


class TestDIFToFodgBasic:
    def test_returns_page_count(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = dif_to_fodg(SAMPLE, dest)
        assert isinstance(count, int) and count >= 0

    def test_output_file_created(self, tmp_path):
        dest = tmp_path / "out.fodg"
        dif_to_fodg(SAMPLE, dest)
        assert dest.exists()
...
class TestDIFToFodgPaths:
    ...
    def test_accepts_string_paths(self, tmp_path):
        dest = tmp_path / "out.fodg"
        count = dif_to_fodg(str(SAMPLE), str(dest))
