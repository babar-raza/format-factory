"""Tests for R1239: PpmDocument encoding and size classification properties.
...
def _make_doc(width: int, height: int, magic: str = "P3", maxval: int = 255) -> PpmDocument:
    """Build a PpmDocument stub with given dimensions and magic."""
...
class TestIsAscii:
    def test_p3_magic_is_ascii(self):
        doc = _make_doc(10, 10, magic="P3")
        assert doc.is_ascii is True

    def test_p6_magic_not_ascii(self):
        doc = _make_doc(10, 10, magic="P6")
        assert doc.is_ascii is False

    def test_is_ascii_inverse_of_is_binary(self):
        doc_p3 = _make_doc(10, 10, magic="P3")
        doc_p6 = _make_doc(10, 10, magic="P6")
        assert doc_p3.is_ascii is True
        assert doc_p3.is_binary is False
        assert doc_p6.is_ascii is False
        assert doc_p6.is_binary is True

    def test_ascii_zero_dimensions(self):
        doc = _make_doc(0, 0, magic="P3")
        assert doc.is_ascii is True
...
class TestCrossPropertyConsistency:
    ...
    def test_ascii_and_large(self):
        doc = _make_doc(1920, 1080, magic="P3")
        assert doc.is_ascii is True
        assert doc.is_large_image is True

    def test_binary_and_landscape_with_edges(self):
        doc = _make_doc(1920, 1080, magic="P6")
        assert doc.is_ascii is False
