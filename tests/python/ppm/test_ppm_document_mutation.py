"""Tests for PpmDocument mutation API: set_pixel() and save_to_file().

Sprint: PPM-PYTHON-MUTATION-001
"""
import tempfile
...
from ppm.ppm_parser import PpmError


SAMPLE_PPM = Path("samples/by-format/ppm/valid/2x2-rgbw.ppm")


class TestSetPixel:
    def test_set_pixel_value(self):
        doc = PpmDocument.from_file(SAMPLE_PPM)
...
class TestMutationRoundtrip:
    def test_set_pixel_roundtrip(self):
        ...
        doc = PpmDocument.from_file(SAMPLE_PPM)
        doc.set_pixel(0, (11, 22, 33))
        with tempfile.TemporaryDirectory() as td:
            dest = Path(td) / "rt.ppm"
            doc.save_to_file(dest)
            reloaded = PpmDocument.from_file(dest)
            assert reloaded._parsed.pixels[0] == (11, 22, 33)

    def test_roundtrip_dimensions_preserved(self):
        doc = PpmDocument.from_file(SAMPLE_PPM)
        original_w, original_h = doc.width, doc.height
        doc.set_pixel(0, (5, 5, 5))
        with tempfile.TemporaryDirectory() as td:
            dest = Path(td) / "rt2.ppm"
            doc.save_to_file(dest)
            reloaded = PpmDocument.from_file(dest)
    ...
    def test_roundtrip_pixel_count_preserved(self):
        ...
        doc.set_pixel(0, (0, 0, 0))
        with tempfile.TemporaryDirectory() as td:
            dest = Path(td) / "rt3.ppm"
            doc.save_to_file(dest)
            reloaded = PpmDocument.from_file(dest)
