"""PPM malformed input and security guard tests.
...
Security guards tested:
  ...
  - Injection guard: magic-byte validation (only P3/P6 accepted)
"""
...
VALID_SAMPLE = _REPO / "samples" / "by-format" / "ppm" / "valid" / "1x1-red.ppm"
WRONG_MAGIC_SAMPLE = _REPO / "samples" / "by-format" / "ppm" / "invalid" / "wrong-magic.ppm"
...
class TestClassAWrongMagic:
    ...
    def test_wrong_magic_sample_raises(self) -> None:
        ...
        with pytest.raises(Exception):
            ppm.parse_ppm_strict(str(WRONG_MAGIC_SAMPLE))

    def test_p1_magic_rejected(self, tmp_path: Path) -> None:
        """P1 (PBM ASCII) magic must be rejected by PPM parser."""
        f = tmp_path / "bad.ppm"
        f.write_text("P1\n2 2\n0 1\n1 0\n", encoding="ascii")
        with pytest.raises(Exception):
            ppm.parse_ppm_strict(str(f))

    def test_empty_file_raises(self, tmp_path: Path) -> None:
        """Empty file must raise an exception."""
        f = tmp_path / "empty.ppm"
        f.write_bytes(b"")
        with pytest.raises(Exception):
            ppm.parse_ppm_strict(str(f))
...
class TestClassCDecodeFailure:
    ...
    def test_truncated_pixel_data(self, tmp_path: Path) -> None:
        """Truncated RGB pixel data must raise an exception."""
        f = tmp_path / "truncated.ppm"
        # 2x2 = 4 pixels = 12 values; only provide 3
        f.write_text("P3\n2 2\n255\n255 0 0\n", encoding="ascii")
        with pytest.raises(Exception):
            ppm.parse_ppm_strict(str(f))

    def test_channel_value_exceeds_maxval(self, tmp_path: Path) -> None:
        """Channel value > maxval must raise an exception."""
        f = tmp_path / "overflow.ppm"
...
class TestSecuritySizeGuard:
    ...
    def test_valid_small_file_parsed(self) -> None:
        ...
        img = ppm.parse_ppm_strict(str(VALID_SAMPLE))
        assert img.width >= 1
        assert img.height >= 1

    def test_probe_does_not_raise_on_valid(self) -> None:
        """probe_ppm on valid file must not raise."""
        info = ppm.probe_ppm(str(VALID_SAMPLE))
        assert info["width"] >= 1
...
class TestSecurityInjectionGuard:
    """Security guard: only P3/P6 magic accepted; all others raise immediately."""

    def test_p2_magic_rejected(self, tmp_path: Path) -> None:
        """P2 (PGM ASCII) magic must be rejected by PPM parser."""
        f = tmp_path / "p2.ppm"
        f.write_text("P2\n1 1\n255\n128\n", encoding="ascii")
        with pytest.raises(Exception):
            ppm.parse_ppm_strict(str(f))
