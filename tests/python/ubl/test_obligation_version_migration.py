...
``migrate_document()`` is grounded directly in that fact, not invented:
it supports ONLY the 2.1-to-2.3 direction and ONLY the 65 covered root
types, refusing everything else rather than guessing.
"""
...
def test_migrate_document_refuses_a_document_not_declaring_2_1():
    document = load(_invoice_bytes(version="2.2"))
    with pytest.raises(UblValidationError, match="2.1"):
        migrate_document(document)
