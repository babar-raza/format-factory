...
**Purpose of this folder:** Contains format test suites, oracle outputs, and fuzz seeds.
Created by developers and the oracle pipeline. Python format tests live in `tests/python/<fmt>/`;
.NET format tests in `tests/net/<fmt>/`; supervisor machinery tests in `tests/supervisor/`.

**To add format tests:** Create `tests/python/<format_id>/test_<format_id>.py` (or
`tests/net/<format_id>/` for .NET). Run with:
`python .venv/Scripts/pytest tests/python/<format_id>/ -q`

**To run the full test suite:** `python .venv/Scripts/pytest tests/ -q --tb=short`

**Source-test parity rule:** Every `src/python/<fmt>/` must have `tests/python/<fmt>/`.
Validated by V91 (`python tools/supervisor/governance_validators_root_struct.py`).

**Producer:** Developer-authored tests + oracle pipeline. Oracle outputs generated by
`python tools/supervisor/execute_oracle.py`.
