...
public class FodtR414GetParagraphCountAndDefaultFontDeepTests : IDisposable
{
    private readonly string _tempDir;
    public FodtR414GetParagraphCountAndDefaultFontDeepTests()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "FodtR414GetParagraphCountAndDefaultFontDeepTests_" + Guid.NewGuid().ToString("N"));
...
<office:document xmlns:office=""urn:oasis:names:tc:opendocument:xmlns:office:1.0""
  ...
  <office:body>
    ...
    [Fact]
    public void AddParagraph_Incremental_CountIncreases()
    {
        var doc = new FodtDocument();
        int initial = doc.ParagraphCount;
        doc.AddParagraph("New Para");
        Assert.Equal(initial + 1, doc.ParagraphCount);
    }
    [Fact]
    public void GetDefaultFontName_WithStyle_ReturnsFont()
    {
        var content = CreateFodt(new[] { "Test" }, "Roboto");
        var path = TempFile("font.fodt");
        File.WriteAllText(path, content);
        var doc = FodtDocument.LoadFile(path);
        Assert.Equal("Roboto", doc.GetDefaultFontName());
    }
    [Fact]
    public void GetPageCount_ValidDoc_ReturnsAtLeastOne()
    {
        var content = CreateFodt(new[] { "Page 1", "Page 2" });
    ...
    [Fact]
    public void SaveLoad_RoundTrip_PreservesParagraphCountAndFont()
    {
        var content = CreateFodt(new[] { "Save", "Load" }, "Ubuntu");
    ...
    [Fact]
    public void Dogfood_UkGovDoc_ParagraphsFontPagesCorrect()
    {
        var ukParas = new[]
        {
            "UK GOVERNMENT ANNUAL REPORT 2024",
            "NHS England - Population: 56,000,000",
            "Budget: £150,000,000,000",
            "HMRC - Taxpayers: 67,000,000",
            "Revenue: £800,000,000,000",
            "DVLA - Licences: 48,000,000",
            "ONS - Population: 67,000,000"
        };
        var content = CreateFodt(ukParas, "Helvetica");
        var path = TempFile("ukgov.fodt");
        File.WriteAllText(path, content);
        var doc = FodtDocument.LoadFile(path);
        Assert.Equal(7, doc.ParagraphCount);
        Assert.Equal("Helvetica", doc.GetDefaultFontName());
        Assert.True(doc.GetPageCount() >= 1);
    }
}