// Tests for FodtDocument.GetPageStyleCount dedicated coverage.
// Sprint: ff-sprint-s406-dotnet-deepening-20260701
// Ledger: PC-FODT-R430

using Xunit;

namespace FormatFactory.Fodt.Tests;

/// <summary>
/// R430: Dedicated tests for FodtDocument.GetPageStyleCount().
/// New document returns non-negative count.
/// ParagraphCount unchanged after GetPageStyleCount.
/// TableCount unchanged after GetPageStyleCount.
/// Idempotent (called twice same result).
/// Return type is int.
/// Dogfood: after adding paragraphs count non-negative.
/// Dogfood: after mixed content count non-negative.
/// Dogfood: loop over documents all non-negative.
/// </summary>
public class FodtR430GetPageStyleCountDedicatedTests
{
    // -------------------------------------------------------------------------
    // Functional tests
    // -------------------------------------------------------------------------

    [Fact]
    public void GetPageStyleCount_NewDocument_ReturnsNonNegative()
    {
        var doc = FodtDocument.CreateNew();
        int count = doc.GetPageStyleCount();
        Assert.True(count >= 0);
    }

    [Fact]
    public void GetPageStyleCount_ParagraphCountUnchanged()
    {
        var doc = FodtDocument.CreateNew();
        int before = doc.ParagraphCount;
        _ = doc.GetPageStyleCount();
        Assert.Equal(before, doc.ParagraphCount);
    }

    [Fact]
    public void GetPageStyleCount_TableCountUnchanged()
    {
        var doc = FodtDocument.CreateNew();
        int before = doc.TableCount;
        _ = doc.GetPageStyleCount();
        Assert.Equal(before, doc.TableCount);
    }

    [Fact]
    public void GetPageStyleCount_Idempotent()
    {
        var doc = FodtDocument.CreateNew();
        int first = doc.GetPageStyleCount();
        int second = doc.GetPageStyleCount();
        Assert.Equal(first, second);
    }

    [Fact]
    public void GetPageStyleCount_IsInt()
    {
        var doc = FodtDocument.CreateNew();
        object result = doc.GetPageStyleCount();
        Assert.IsType<int>(result);
    }

    // -------------------------------------------------------------------------
    // Dogfood pipeline
    // -------------------------------------------------------------------------

    [Fact]
    public void DogfoodPipeline_AfterParagraphs_CountNonNegative()
    {
        var doc = FodtDocument.CreateNew();
        doc.AddParagraph("First paragraph");
        doc.AddParagraph("Second paragraph");
        Assert.True(doc.GetPageStyleCount() >= 0);
    }

    [Fact]
    public void DogfoodPipeline_AfterMixedContent_CountNonNegative()
    {
        var doc = FodtDocument.CreateNew();
        doc.AddParagraph("Intro");
        doc.AddTable(2, 2);
        doc.AddParagraph("Conclusion");
        Assert.True(doc.GetPageStyleCount() >= 0);
    }

    [Fact]
    public void DogfoodPipeline_MultipleDocuments_AllNonNegative()
    {
        for (int i = 0; i < 3; i++)
        {
            var doc = FodtDocument.CreateNew();
            doc.AddParagraph($"Document {i}");
            Assert.True(doc.GetPageStyleCount() >= 0);
        }
    }
}
