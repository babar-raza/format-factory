...
public class FodtR413GetParagraphCountAndPageCountDeepTests : IDisposable
{
    private readonly string _tempDir;
    public FodtR413GetParagraphCountAndPageCountDeepTests()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "FodtR413GetParagraphCountAndPageCountDeepTests_" + Guid.NewGuid().ToString("N"));
...
<office:document xmlns:office=""urn:oasis:names:tc:opendocument:xmlns:office:1.0""
  ...
  <office:body>
    ...
    [Fact]
    public void AddParagraph_Multiple_AddsCorrectly()
    {
    ...
    {
        var content = CreateFodt(new[] { "Test" }, "DejaVu Sans");
        var path = TempFile("font.fodt");
        File.WriteAllText(path, content);
        var doc = FodtDocument.LoadFile(path);
        Assert.Equal("DejaVu Sans", doc.GetDefaultFontName());
    }
    [Fact]
    public void SaveLoad_RoundTrip_PreservesParagraphs()
    {
        var content = CreateFodt(new[] { "Round", "Trip", "Test" });
    ...
    [Fact]
    public void Dogfood_UkGovReport_ParagraphsPagesFontCorrect()
    {
        var ukParas = new[]
        {
            "UK GOVERNMENT REPORT 2024",
            "NHS England - Population Served: 56,000,000",
            "Budget Allocation: £150,000,000,000",
            "HMRC - Registered Taxpayers: 67,000,000",
            "Annual Revenue Collected: £800,000,000,000",
            "DVLA - Active Driving Licences: 48,000,000",
            "ONS - UK Population Estimate: 67,000,000",
            "DEFRA - Environmental Budget: £8,000,000,000"
        };
        var content = CreateFodt(ukParas, "Arial");
