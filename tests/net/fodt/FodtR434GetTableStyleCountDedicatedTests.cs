// Tests for FodtDocument.GetTableStyleCount dedicated coverage.
// Sprint: ff-sprint-s410-dotnet-deepening-20260701
// Ledger: PC-FODT-R434

using Xunit;

namespace FormatFactory.Fodt.Tests;

/// <summary>
/// R434: Dedicated tests for FodtDocument.GetTableStyleCount().
/// New document returns non-negative count.
/// ParagraphCount unchanged after GetTableStyleCount.
/// TableCount unchanged after GetTableStyleCount.
/// Idempotent (called twice same result).
/// Return type is int.
/// Dogfood: after adding paragraphs count non-negative.
/// Dogfood: after mixed content count non-negative.
/// Dogfood: loop over documents all non-negative.
/// </summary>
public class FodtR434GetTableStyleCountDedicatedTests
{
    // -------------------------------------------------------------------------
    // Functional tests
    // -------------------------------------------------------------------------

    [Fact]
    public void GetTableStyleCount_NewDocument_ReturnsNonNegative()
    {
        var doc = FodtDocument.CreateNew();
        int count = doc.GetTableStyleCount();
        Assert.True(count >= 0);
    }

    [Fact]
    public void GetTableStyleCount_ParagraphCountUnchanged()
    {
        var doc = FodtDocument.CreateNew();
        int before = doc.ParagraphCount;
        _ = doc.GetTableStyleCount();
        Assert.Equal(before, doc.ParagraphCount);
    }

    [Fact]
    public void GetTableStyleCount_TableCountUnchanged()
    {
        var doc = FodtDocument.CreateNew();
        int before = doc.TableCount;
        _ = doc.GetTableStyleCount();
        Assert.Equal(before, doc.TableCount);
    }

    [Fact]
    public void GetTableStyleCount_Idempotent()
    {
        var doc = FodtDocument.CreateNew();
        int first = doc.GetTableStyleCount();
        int second = doc.GetTableStyleCount();
        Assert.Equal(first, second);
    }

    [Fact]
    public void GetTableStyleCount_IsInt()
    {
        var doc = FodtDocument.CreateNew();
        object result = doc.GetTableStyleCount();
        Assert.IsType<int>(result);
    }

    // -------------------------------------------------------------------------
    // Dogfood pipeline
    // -------------------------------------------------------------------------

    [Fact]
    public void DogfoodPipeline_AfterParagraphs_CountNonNegative()
    {
        var doc = FodtDocument.CreateNew();
        doc.AddParagraph("First paragraph");
        doc.AddParagraph("Second paragraph");
        Assert.True(doc.GetTableStyleCount() >= 0);
    }

    [Fact]
    public void DogfoodPipeline_AfterMixedContent_CountNonNegative()
    {
        var doc = FodtDocument.CreateNew();
        doc.AddParagraph("Intro");
        doc.AddTable(2, 2);
        doc.AddParagraph("Conclusion");
        Assert.True(doc.GetTableStyleCount() >= 0);
    }

    [Fact]
    public void DogfoodPipeline_MultipleDocuments_AllNonNegative()
    {
        for (int i = 0; i < 3; i++)
        {
            var doc = FodtDocument.CreateNew();
            doc.AddParagraph($"Document {i}");
            Assert.True(doc.GetTableStyleCount() >= 0);
        }
    }
}
