...
for sample in gnumeric_files:
    ...
    try:
        ...
        sheet_count = gnumeric.get_sheet_count(model)
        cell_count = gnumeric.get_cell_count(model)
        values = gnumeric.extract_values(model)
        metadata = gnumeric.get_sheet_metadata(model)
